...
**Blog strip + FAQ (built 2026-07-05 evening, George: "we have blogs"):** two new sections between team strip and final CTA. Blog strip = 3 REAL posts from `src/views/blogs/blogPosts.ts` (loan-shark credit record / contact-list collateral / $15-loans-are-infrastructure) as compact list cards with their real cover thumbs (resized ≤160px in `assets/blog/`), linking to `/blogs/<slug>` (works once landing lives on the app domain; 404s in the standalone prototype). FAQ = 4 native `<details>` items answering the trust questions (can't-pay / who funds / pesos / why-$15) — **FAQ copy + peso answer need George's sign-off** (peso answer echoes the unapproved peso-strip strawman). ⚠️ The blog thumbs are hippo covers — they're the posts' real covers, not section mascots, but George may want them gone under the no-hippos rule; swap to text-only cards is trivial.
