...
**Blog strip + FAQ (built 2026-07-05 evening, George: "we have blogs"):** two new sections between team strip and final CTA. Blog strip = 3 REAL posts from `src/views/blogs/blogPosts.ts` (loan-shark credit record / contact-list collateral / $15-loans-are-infrastructure), restyled to **MAKE's bottom-section card anatomy** (George asked for it explicitly): big lavender cover panel → bold title → dek clamped to 3 lines → byline (logo avatar + Moodeng + date · read time) → hashtag category chip; "See all +" link in the section header row. Covers are WebP-with-alpha in `assets/blog/` (~55KB total, white bgs flood-filled out). Links go to `/blogs/<slug>` (work once landing lives on the app domain; 404 in the standalone prototype). Gotcha discovered: `loading="lazy"` on a zero-intrinsic-width img never fetches in data-saver-ish browsers — covers load eagerly instead. FAQ = 4 native `<details>` items answering the trust questions (can't-pay / who funds / pesos / why-$15) — **FAQ copy + peso answer need George's sign-off** (peso answer echoes the unapproved peso-strip strawman). ⚠️ The blog thumbs are hippo covers — they're the posts' real covers, not section mascots, but George may want them gone under the no-hippos rule; swap to text-only cards is trivial.
