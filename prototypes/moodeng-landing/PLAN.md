...
## 2026-07-05 NIGHT — header/footer/tips finalized with George live

- **Header (FINAL after 4 iterations — George's taste path is instructive):** Tilt-style dark bar `#100626`, 3D hippo logo (`assets/brand/moodeng-logo-small.png`, 3.8KB) + "Moodeng Credit" white 17px left, solid-purple hamburger pill right, sticky. Rejected along the way: white Klarna bar w/ "Open app" pill ("I just wanted moodeng → Moodeng"), then a white minimal Klarna bar (menu-pill/wordmark/account) — "too much"; dark + quiet won. Hamburger is VISUAL ONLY (no menu exists).
- **Brand naming:** "Moodeng Credit" in header lockup, footer brand, © line. The giant clipped footer wordmark stays plain "Moodeng" — George: "i like that a lot", don't touch it.
- **Footer:** Klarna hierarchy — centered grid columns in the site wrap, hover states, giant white `f-mega` wordmark clipped at 0.62em with symmetric horizontal bleed (flex justify-center trick; plain text-align center spills right-only). SEC placeholder comment + honest-links-only rule unchanged (no fake About/Careers columns).
- **Tips carousel:** George generated three full CARD DESIGNS (bluish sky, professional) — the cards now ship as single whole-card JPGs (`assets/tips/tip-N-card.jpg`, ~100KB, alt text carries the titles). My first pass (crop illustration, re-render body text as HTML) was REJECTED: "you put text and stuff on top of it, weird." Lesson: when George supplies a finished design, use it verbatim. Crop recipe for his ChatGPT card exports (gray margin, baked corner radius + drop shadow): sky-anchored edge detection + shadow-anchored bottom, inset 14px, in `tools/` scripts' history. Bluish palette = "figure that out later"; text now baked in pixels (regenerate to edit); articles still INVENTED, links `#`.
- **Hero video pipeline (George's Veo experiments, settled workflow):** generators CANNOT render our UI pixel-exact — Veo ignored the attached screen image and invented a garbled "$5,000 / Freeze" loan app (off-brand poison: our story is $15). Division of labor: **Veo generates the world with a BLANK WHITE screen; we composite the real card in post.** Tooling now in `tools/`: `strip_bg.py` (checkerboard removal for 3D assets), `composite.py` (still), `composite_video.py` (per-frame white-screen tracking; EMA-smoothed; stutters on high-swing takes — ask George for "phone stays almost still, only charms sway"). Flat card screen renders via `_shot.html?mode=phone` + headless Chrome. `moodeng-screen-for-video.png` (1080×2280 screen image) exists in scratchpad-history for generator experiments — they failed, don't retry that route.
- **Current placeholder debt:** `assets/hero/hero-loop.mp4` still = my stuttery tracked composite; George owes the calm white-screen Veo take. Hero H1/sub copy ("Small loans that treat you right.") is mine, needs his sign-off. Peso hint on GET BACK chip hides for non-PH IPs (by design).
...
**Tips carousel + FAQ (2026-07-05 evening; tips REBUILT FROM SCRATCH after George rejected two blog-strip versions):** two sections between team strip and final CTA. Tips carousel (`.sec-tips`) = **direct copy of MAKE's bottom section in Moodeng skin** — George: "simply copy the kbank make ones with our style, do not use our blog stuff at all." Anatomy per MAKE mobile: header (title left, "See all +" right) → horizontal scroll-snap carousel, one card + next peeking → card = sky cover with title/subtitle baked in + logo badge top-right + clouds (George supplies the cover illustration scenes; `.art` slot is an HTML comment) → white body (title, 2-line clamped excerpt, byline, hashtag chips) → progress dots (IO + scroll-listener, both, because nested-scroller events are throttled in some environments). ⚠️ **All three tip articles (titles/excerpts/dates/read times) are INVENTED PLACEHOLDERS and link to `#` — do not ship until real.** Earlier real-blog-posts version (commits 0e899e76c/229e79909) was rejected — don't resurrect it; `assets/blog/` covers deleted. Lazy-load gotcha kept for the record: `loading="lazy"` on a zero-intrinsic-width img never fetches in data-saver browsers. FAQ = 4 native `<details>` items answering the trust questions (can't-pay / who funds / pesos / why-$15) — **FAQ copy + peso answer need George's sign-off** (peso answer echoes the unapproved peso-strip strawman). ⚠️ The blog thumbs are hippo covers — they're the posts' real covers, not section mascots, but George may want them gone under the no-hippos rule; swap to text-only cards is trivial.
...
- **ALL section mascots OFF for now (George, 2026-07-05 evening — superseded the earlier "top ¾ hippo-free" call).** Every hippo (community trio, dark-section lock, ladder trophy, team thumbs-up, final-CTA party) is COMMENTED OUT in index.html, not deleted — one-line restores when George decides mascot placement (likely after the hero exists). Only the brand-logo hippo in the header/footer remains.
