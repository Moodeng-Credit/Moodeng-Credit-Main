...
// Heavy libs — code-split so they only load when this tab is opened.
const SelfLendingGraph = lazy(() => import('./SelfLendingGraph'));
const SelfLendingMap = lazy(() => import('./SelfLendingMap'));
const FraudAlertQueue = lazy(() => import('./FraudAlertQueue'));

type ToggleWhitelist = (account: DetectionAccount) => void;
   ...
   return (
      <span className="inline-flex items-center gap-2 rounded-full bg-[#241044] px-3 py-1 text-base font-bold text-white">
         ...
         {onToggleWhitelist ? (
            <button
               type="button"
               onClick={() => onToggleWhitelist(account)}
               title={account.whitelisted ? 'Remove from whitelist' : 'Mute — whitelist as known-good'}
               className="rounded-full bg-[#34234f] px-2 py-0.5 text-xs font-black uppercase text-[#cfc6dd] hover:bg-[#3d1f6e]"
            >
               {account.whitelisted ? 'unmute' : 'mute'}
            </button>
         ) : null}
      </span>
   );
         ...
         <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-[#241044] p-4">
               ...
               <div className="mt-2 flex flex-wrap gap-2">
                  {borrowers.length ? (
                     borrowers.map((a) => (
                        <AccountChip key={a.user_id} account={a} onToggleWhitelist={onToggleWhitelist} />
                     ))
                  ) : (
...
function LoanCard({ loan, onToggleWhitelist }: { loan: SelfLendingLoan; onToggleWhitelist?: ToggleWhitelist }) {
      ...
      >
         ...
         <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-[#241044] p-4">
               ...
               <div className="mt-2">
                  <AccountChip account={loan.borrower} onToggleWhitelist={onToggleWhitelist} />
               </div>
               {loan.borrower.email ? (
            ...
            <div className="rounded-2xl bg-[#241044] p-4">
               ...
               <div className="mt-2">
                  <AccountChip account={loan.lender} onToggleWhitelist={onToggleWhitelist} />
               </div>
               {loan.lender.email ? (
...
export default function SelfLendingSection() {
   ...
   // Mute (whitelist) / unmute an account, then refresh so badges update.
   const toggleWhitelist = useCallback<ToggleWhitelist>(
      async (account) => {
         try {
            if (account.whitelisted) await removeFromWhitelist(account.user_id);
            else await addToWhitelist(account.user_id);
            await load();
         } catch (e) {
            setError(e instanceof Error ? e.message : String(e));
         }
   ...
   const hostingIps = useMemo(() => data?.hosting_ips ?? [], [data]);
   ...
   return (
      <section className="space-y-6">
         ...
         <div className="space-y-4">
            ...
            {loans.length ? (
               <div className="grid gap-5 xl:grid-cols-2">
                  {loans.map((loan) => (
                     <LoanCard key={loan.loan_id} loan={loan} onToggleWhitelist={toggleWhitelist} />
                  ))}
               </div>
            ) : (
         ...
         {/* Alert queue — triage the daily scan's findings */}
         <Suspense
            fallback={
         ...
         {/* Login map — geographic hotspots from GeoLite2 */}
         <div className="space-y-3">
            <h3 className="text-3xl font-black">Login map</h3>
            <p className="-mt-1 text-lg font-bold text-[#a89bb8]">
               Where accounts log in from. <span className="text-[#ff6b7a]">Red</span> = multiple accounts one spot,{' '}
               <span className="text-[#f5a524]">amber</span> = datacenter/VPN.
            </p>
            <Suspense
               fallback={
         ...
         {/* Datacenter / hosting-IP logins */}
         <HostingSection hostingIps={hostingIps} onToggleWhitelist={toggleWhitelist} />
      </section>
   );
}
...
function HostingSection({
   ...
   hostingIps: HostingIp[];
   onToggleWhitelist: ToggleWhitelist;
}) {
   return (
      <div className="space-y-4">
         <h3 className="text-3xl font-black">
            Datacenter / hosting logins{' '}
            <span className="text-xl font-bold text-[#a89bb8]">({hostingIps.length})</span>
         </h3>
         <p className="-mt-2 text-lg font-bold text-[#a89bb8]">
            Accounts logging in from a hosting/cloud ASN — a &ldquo;personal&rdquo; account on a datacenter IP is a flag.
         </p>
         {hostingIps.length ? (
            <div className="grid gap-3 sm:grid-cols-2">
               {hostingIps.map((h) => (
                  <article
                     key={`${h.user_id}:${h.asn_org}`}
                     className="flex items-center justify-between gap-3 rounded-2xl border border-[#2a1453] bg-[#1c0a3a] p-4"
                  >
                     <div className="min-w-0">
                        <div className="text-xl font-black">{h.username ?? h.user_id.slice(0, 8)}</div>
                        <div className="break-words text-base font-bold text-[#a89bb8]">
                           {h.asn_org ?? 'unknown ASN'}
                           {h.country ? ` · ${h.country}` : ''} · {h.login_count}×
                        </div>
                     </div>
                     <AccountChip
                        account={{ user_id: h.user_id, username: h.username, whitelisted: h.whitelisted }}
            ...
            <div className="space-y-4">
               {sorted.map((item) => (
                  <article key={item.key} className="rounded-3xl border border-[#2a1453] bg-[#1c0a3a] p-6">
                     ...
                     <RoleSplit accounts={item.accounts} onToggleWhitelist={onToggleWhitelist} />
                  </article>
               ))}
            </div>
         ) : (
