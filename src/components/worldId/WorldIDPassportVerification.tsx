...
const WORLD_ID_VERIFICATION_METHOD = 'passport';
...
function VerificationLaunchOverlay({ state, onRetryOpen, onCancel }: VerificationLaunchOverlayProps) {
   ...
   return (
      <div
         ...
         aria-modal="true"
         aria-live="polite"
         aria-labelledby="world-id-passport-launch-title"
         aria-describedby="world-id-passport-launch-description"
      >
         <div className="w-full max-w-[398px] rounded-md-lg border border-md-neutral-400 bg-md-neutral-100 px-md-4 py-md-5 text-center shadow-md-card">
            ...
            <div className="mt-md-3 flex flex-col gap-md-1">
               <h2 id="world-id-passport-launch-title" className="text-[24px] font-[590] leading-[1.2] tracking-normal text-md-heading">
                  {isFallback ? "World ID didn't open?" : 'Opening World ID...'}
               </h2>
               <p id="world-id-passport-launch-description" className="text-md-b2 font-normal leading-[1.5] tracking-normal text-md-neutral-1200">
                  {isFallback
                     ...
                     : "World App will open to verify your passport or ID. If you don't have World App yet, you'll be guided to install it first."}
               </p>
            </div>
...
export default function WorldIDPassportVerification({
   ...
   const refreshUserUntilWorldIdActive = useCallback(async () => {
      for (let attempt = 0; attempt < STATUS_REFRESH_RETRIES; attempt += 1) {
         ...
         if (refreshedUser.isWorldIdPassport === 'ACTIVE') {
            return;
         }
      }
   ...
   const createWorldIdRequest = useCallback(async () => {
      ...
      }).constraints(CredentialRequest('passport'));
   ...
   useEffect(() => {
      ...
      void prepareWorldIdRequest().catch((error) => {
         if (isMounted) {
            console.error('[WorldIDPassport] prepareWorldIdRequest error:', error instanceof Error ? error.message : error);
         }
      });
   ...
   const handleVerify = useCallback(async (proof: IDKitResult) => {
      ...
      try {
         ...
         const res = await fetch(`${apiUrl}/verify-worldid`, {
            ...
            body: JSON.stringify({ type: 'verify', proof, method: WORLD_ID_VERIFICATION_METHOD })
         });
         ...
         console.error('[WorldIDPassport] handleVerify error:', error);
         throw error;
      }
   ...
   const openExternalWorldId = useCallback((connectorURI: string) => {
      try {
         ...
         console.error('[WorldIDPassport] openExternalWorldId error:', error);
         return false;
      }
   }, [clearLaunchFallbackTimer]);
   ...
   const beginPollingWorldIdRequest = useCallback(
      (request: PreparedWorldIdRequest) => {
         ...
         void (async () => {
            ...
            if (pollRunRef.current === runId) {
               setVerificationLaunchState('idle');
               console.error('[WorldIDPassport] beginPollingWorldIdRequest error:', error);
               showToastByConfig('server_error');
            }
         });
      },
   ...
   const handleStartWorldIdLaunch = useCallback(() => {
      ...
      if (needsCurrentTabWorldAppLaunch()) {
         ...
         prepareWorldIdRequest()
            ...
            .catch((error) => {
               ...
               console.error('[WorldIDPassport] mobile launch error:', error instanceof Error ? error.message : error);
               showToastByConfig('server_error');
            })
            .finally(() => {
               preparingRef.current = false;
      ...
      try {
         ...
         console.error('[WorldIDPassport] placeholder launch window error:', error);
      }
      ...
      prepareWorldIdRequest()
         ...
         .catch((error) => {
            setVerificationLaunchState('idle');
            if (error instanceof Error && error.message === 'WORLDID_ALREADY_USED') return;
            console.error('[WorldIDPassport] handleStartWorldIdLaunch error:', error instanceof Error ? error.message : error);
            showToastByConfig('server_error');
         })
         .finally(() => {
            preparingRef.current = false;
