import { type JSX } from 'react';

import Image from 'next/image';

import ActionButton from '@/components/ui/ActionButton';

import { financialInclusionButtons } from '@/config/buttonConfig';

export default function FinancialInclusionSection(): JSX.Element {
   return (
      <div className="relative w-[1440px] h-[1168px] bg-white rounded-[60px] overflow-hidden">
         <div className="absolute w-[1236px] h-[42px] top-[60px] left-[49px] bg-white">
            <p className="absolute w-[660px] h-8 top-1 left-0 [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#171420] text-3xl tracking-[0] leading-8 whitespace-nowrap">
               POWER TOOLS FOR FINANCIAL INCLUSION
            </p>
            <div className="absolute w-[209px] h-[42px] top-0 left-[1027px] rounded-[100px]">
               <div className="absolute w-[209px] h-[42px] top-0 left-0 bg-[#171420] rounded-[100px] overflow-hidden">
                  <div className="flex-shrink-0">
                     {financialInclusionButtons.map((button) => (
                        <ActionButton key={button.text} button={button} />
                     ))}
                  </div>
               </div>
            </div>
         </div>
         <div className="absolute w-[1236px] h-[982px] top-[126px] left-[102px]">
            <div className="absolute w-[1236px] h-[982px] top-0 left-0">
               <div className="absolute w-[1236px] h-[668px] top-0 left-0">
                  <div className="absolute w-[401px] h-[668px] top-0 left-0 rounded-[30px] overflow-hidden bg-[url(https://c.animaapp.com/VPWnEuWR/img/d7b165iclurvlhgwbaawvz1kfe-svg.svg)] bg-cover bg-[50%_50%]">
                     <div className="w-[337px] h-8 top-[39px] left-[30px] [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#17142080] text-xl leading-8 whitespace-nowrap absolute tracking-[0]">
                        BEST WAY TO BUILD CREDIT
                     </div>
                     <div className="absolute w-[341px] h-[210px] top-[88px] left-[30px]">
                        <p className="absolute w-[364px] h-[203px] top-0.5 left-0 [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#171420] text-[40px] tracking-[0] leading-[42px]">
                           Direct Lender-to-Borrower $15 microloans. Unlock more credit when you repay.
                        </p>
                     </div>
                     <div className="absolute w-[341px] h-[324px] top-[314px] left-[30px]">
                        <div className="relative w-[310px] h-[324px] left-4">
                           <div className="w-[309px] h-[324px]">
                              <div className="h-[324px]">
                                 <div className="relative w-[309px] h-[324px]">
                                    <Image
                                       className="absolute w-[300px] h-[307px] top-[17px] left-0"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-3.svg"
                                       width={300}
                                       height={307}
                                    />
                                    <Image
                                       className="absolute w-[264px] h-14 top-[155px] left-[18px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-4.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[264px] h-14 top-[221px] left-[18px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-5.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[264px] h-14 top-[89px] left-[18px]"
                                       alt=""
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-6.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       alt=""
                                       loading="lazy"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-11.svg"
                                       className="absolute w-[34px] h-[34px] top-[167px] left-[229px]"
                                       width={34}
                                       height={34}
                                    />
                                    <Image
                                       alt=""
                                       loading="lazy"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-12.svg"
                                       className="absolute w-[34px] h-[34px] top-[232px] left-[229px]"
                                       width={34}
                                       height={34}
                                    />
                                    <Image
                                       alt=""
                                       className="absolute w-8 h-7 top-[53px] left-[230px]"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-9.svg"
                                       width={32}
                                       height={28}
                                    />
                                    <Image
                                       className="absolute w-[127px] h-[59px] top-0 left-[183px]"
                                       alt=""
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-10.svg"
                                       width={127}
                                       height={59}
                                    />
                                    <div className="h-[22px] top-[1.3rem] left-[225px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg leading-[normal] absolute tracking-[0]">
                                       Start
                                    </div>
                                 </div>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
                  <div className="absolute w-[401px] h-[668px] top-0 left-[417px] rounded-[30px] overflow-hidden">
                     <div className="relative h-[668px] rounded-[30px]">
                        <div className="absolute w-[401px] h-[668px] top-0 left-0 rounded-[30px] overflow-hidden">
                           <div className="h-[668px] overflow-hidden">
                              <div className="relative w-[454px] h-[668px] left-[-26px]">
                                 <div className="relative w-[401px] h-[668px] left-[26px] bg-[url(https://c.animaapp.com/VPWnEuWR/img/clip-path-group.png)] bg-[100%_100%]">
                                    <Image
                                       className="absolute w-[401px] h-72 top-[380px] left-0 object-cover"
                                       alt="Screenshot"
                                       src="https://c.animaapp.com/VPWnEuWR/img/screenshot-2024-09-21-at-20-49-58-1@2x.png"
                                       width={401}
                                       height={288}
                                    />
                                 </div>
                              </div>
                           </div>
                        </div>
                        <div className="absolute w-[269px] h-8 top-[39px] left-[30px] [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#17142080] text-xl tracking-[0] leading-8 whitespace-nowrap">
                           CHEAPER LOANS
                        </div>
                        <p className="absolute w-[353px] h-[161px] top-[101px] left-[30px] [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#171420] text-[40px] tracking-[0] leading-[42px]">
                           Enjoy lower rates as your credit score improves
                        </p>
                     </div>
                  </div>
                  <div className="absolute w-[401px] h-[668px] top-0 left-[835px] rounded-[30px] overflow-hidden bg-[url(https://c.animaapp.com/VPWnEuWR/img/sdg4rsvsemfs6rn1pmnigbb7zdu-svg.svg)] bg-cover bg-[50%_50%]">
                     <div className="absolute w-[306px] h-8 top-[39px] left-[30px] [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#17142080] text-xl tracking-[0] leading-8 whitespace-nowrap">
                        MICRO-LOAN MANAGEMENT
                     </div>
                     <p className="absolute w-[341px] h-[203px] top-[90px] left-[30px] [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#171420] text-[40px] tracking-[0] leading-[42px]">
                        Get funded in a digital wallet like Metamask, with stablecoins like USDT or USDC
                     </p>
                     <div className="absolute w-[341px] h-[301px] top-[314px] left-[30px]">
                        <div className="relative w-[318px] h-[297px] top-1 left-3">
                           <div className="relative h-[297px]">
                              <div className="absolute w-[318px] h-[246px] top-0 left-0">
                                 <div className="absolute w-[318px] h-[246px] top-0 left-0">
                                    <div className="absolute w-[165px] h-[85px] top-0 left-[15px]">
                                       <Image
                                          className="absolute w-[35px] h-[31px] top-[54px] left-[34px]"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-11.svg"
                                          width={35}
                                          height={31}
                                       />
                                       <Image
                                          className="absolute w-[165px] h-[65px] top-0 left-0"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-12.svg"
                                          width={165}
                                          height={65}
                                       />
                                       <div className="top-[23px] left-[18px] absolute h-[22px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
                                          No Documents
                                       </div>
                                    </div>
                                    <div className="absolute w-[177px] h-[85px] top-[161px] left-0">
                                       <Image
                                          className="absolute w-[35px] h-[31px] top-[54px] left-[93px]"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-13.svg"
                                          width={35}
                                          height={31}
                                       />
                                       <Image
                                          className="absolute w-[177px] h-[65px] top-0 left-0"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-14.svg"
                                          width={177}
                                          height={65}
                                       />
                                       <div className="top-[22px] left-[45px] absolute h-[22px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
                                          No Banks
                                       </div>
                                    </div>
                                    <div className="absolute w-[204px] h-[85px] top-[84px] left-[114px]">
                                       <Image
                                          className="absolute w-[35px] h-[31px] top-[54px] left-[106px]"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-15.svg"
                                          width={35}
                                          height={31}
                                       />
                                       <Image
                                          className="absolute w-[204px] h-[65px] top-0 left-0"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-16.svg"
                                          width={204}
                                          height={65}
                                       />
                                       <div className="absolute h-[22px] top-[23px] left-[34px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
                                          Worth Learning
                                       </div>
                                    </div>
                                 </div>
                                 <div className="absolute w-[45px] h-[45px] top-[95px] left-[46px] bg-[url(https://c.animaapp.com/VPWnEuWR/img/vector-17.svg)] bg-[100%_100%]">
                                    <div className="absolute w-[23px] h-[15px] top-[18px] left-[13px]">
                                       <Image
                                          className="absolute w-[22px] h-3.5 top-0.5 left-px"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-18.svg"
                                          width={100}
                                          height={100}
                                       />
                                       <Image
                                          className="absolute w-[5px] h-1.5 top-0 left-0"
                                          alt="Vector"
                                          src="https://c.animaapp.com/VPWnEuWR/img/vector-19.svg"
                                          width={100}
                                          height={100}
                                       />
                                    </div>
                                    <Image
                                       className="absolute w-[5px] h-1.5 top-[13px] left-[23px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-20.svg"
                                       width={100}
                                       height={100}
                                    />
                                 </div>
                                 <div className="absolute w-[45px] h-[45px] top-[173px] left-[218px] bg-[url(https://c.animaapp.com/VPWnEuWR/img/vector-21.svg)] bg-[100%_100%]">
                                    <Image
                                       className="absolute w-6 h-2.5 top-[23px] left-2.5"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-22.svg"
                                       width={24}
                                       height={8}
                                    />
                                    <Image
                                       className="absolute w-[5px] h-[7px] top-3.5 left-4"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-23.svg"
                                       width={5}
                                       height={7}
                                    />
                                    <Image
                                       className="absolute w-[5px] h-[7px] top-4 left-[26px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-24.svg"
                                       width={5}
                                       height={7}
                                    />
                                 </div>
                              </div>
                              <div className="absolute w-[45px] h-[45px] top-[249px] left-[84px] bg-[url(https://c.animaapp.com/VPWnEuWR/img/vector-25.svg)] bg-[100%_100%]">
                                 <Image
                                    className="absolute w-[25px] h-[9px] top-6 left-[11px]"
                                    alt="Vector"
                                    src="https://c.animaapp.com/VPWnEuWR/img/vector-26.svg"
                                    width={25}
                                    height={9}
                                 />
                                 <Image
                                    className="absolute w-1 h-[7px] top-[15px] left-3.5"
                                    alt="Vector"
                                    src="https://c.animaapp.com/VPWnEuWR/img/vector-27.svg"
                                    width={100}
                                    height={100}
                                 />
                                 <Image
                                    className="absolute w-1 h-[7px] top-3.5 left-[25px]"
                                    alt="Vector"
                                    src="https://c.animaapp.com/VPWnEuWR/img/vector-28.svg"
                                    width={100}
                                    height={100}
                                 />
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
               <div className="absolute w-[1236px] h-[298px] top-[684px] left-0 rounded-[30px] overflow-hidden bg-[url(https://c.animaapp.com/VPWnEuWR/img/ibsgfmymxgphasvaumtopmmos-svg.svg)] bg-cover bg-[50%_50%]">
                  <div className="relative w-[1206px] h-[258px] top-10 left-[30px]">
                     <div className="absolute w-[302px] h-[168px] top-0 left-0">
                        <div className="absolute w-[182px] h-8 -top-px left-0 [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#17142080] text-xl tracking-[0] leading-8 whitespace-nowrap">
                           SET YOUR TERMS
                        </div>
                        <p className="absolute w-[289px] h-[119px] top-11 left-0 [font-family:'PP_Telegraf-Regular',Helvetica] font-normal text-[#171420] text-[40px] tracking-[0] leading-[42px]">
                           Get the Freedom to Choose Your...
                        </p>
                     </div>
                     <div className="absolute w-[894px] h-[258px] top-0 left-[312px] overflow-hidden">
                        <div className="absolute w-[291px] h-[258px] top-0 left-0">
                           <div className="absolute w-[291px] h-[258px] top-0 left-0 overflow-hidden">
                              <div className="relative w-[290px] h-[234px] top-[26px] rounded-[30px_30px_0px_0px]">
                                 <div className="relative h-[234px]">
                                    <Image
                                       className="absolute w-[290px] h-[232px] top-0 left-0"
                                       alt=""
                                       src="https://c.animaapp.com/VPWnEuWR/img/image.svg"
                                       width={290}
                                       height={232}
                                    />
                                 </div>
                              </div>
                           </div>
                           <div className="absolute w-[204px] h-14 top-9 left-[58px] [font-family:'Cabin_Condensed',Helvetica] font-bold text-white text-[40px] tracking-[0]">
                              Interest Rate
                           </div>
                        </div>
                        <div className="absolute w-[306px] h-[258px] top-0 left-[307px] overflow-hidden">
                           <div className="relative w-[290px] h-[234px] top-6 left-3.5">
                              <div className="relative h-[234px] bg-[url(https://c.animaapp.com/VPWnEuWR/img/vector-81.svg)] bg-[100%_100%]">
                                 <div className="absolute w-[72px] h-[74px] top-[81px] left-[34px]">
                                    <Image
                                       className="absolute w-[71px] h-[71px] top-[3px] left-0"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-30.svg"
                                       width={71}
                                       height={71}
                                    />
                                    <Image
                                       className="absolute w-11 h-[71px] top-[3px] left-[26px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-31.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[21px] h-[21px] top-0 left-[51px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-32.svg"
                                       width={21}
                                       height={21}
                                    />
                                 </div>
                                 <div className="absolute w-56 h-14 top-3 left-[34px] [font-family:'Cabin_Condensed',Helvetica] font-bold text-white text-[40px] tracking-[0]">
                                    Payback Time
                                 </div>
                                 <div className="absolute w-32 h-[125px] top-[92px] left-[116px]">
                                    <Image
                                       className="absolute w-[82px] h-[82px] top-2 left-3"
                                       alt="Icon clock"
                                       src="https://c.animaapp.com/VPWnEuWR/img/---icon--clock--8@2x.png"
                                       width={82}
                                       height={82}
                                    />
                                    <Image
                                       className="absolute w-32 h-[125px] top-0 left-0"
                                       alt="Icon clock"
                                       src="https://c.animaapp.com/VPWnEuWR/img/---icon--clock--1@2x.png"
                                       width={100}
                                       height={100}
                                    />
                                 </div>
                              </div>
                           </div>
                        </div>
                        <div className="absolute w-[300px] h-[258px] top-0 left-[629px] overflow-hidden">
                           <div className="relative w-[266px] h-[245px] top-6 left-3.5">
                              <div className="h-[245px] overflow-hidden">
                                 <div className="relative w-[290px] h-[234px]">
                                    <Image
                                       className="absolute w-[252px] h-[234px] top-0 left-0"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-83.svg"
                                       width={252}
                                       height={234}
                                    />
                                    <Image
                                       className="absolute w-[74px] h-[74px] top-[88px] left-[35px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-34.svg"
                                       width={74}
                                       height={74}
                                    />
                                    <Image
                                       className="absolute w-[37px] h-[34px] top-[88px] left-[72px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-35.svg"
                                       width={37}
                                       height={34}
                                    />
                                    <Image
                                       className="absolute w-[22px] h-[22px] top-[84px] left-[88px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-36.svg"
                                       width={22}
                                       height={22}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-4 top-[218px] left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-37.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-[42px] top-48 left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-38.svg"
                                       width={98}
                                       height={42}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-11 top-[165px] left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-39.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-11 top-[137px] left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-40.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-11 top-[111px] left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-41.svg"
                                       width={100}
                                       height={100}
                                    />
                                    <Image
                                       className="absolute w-[98px] h-[38px] top-[93px] left-[148px]"
                                       alt="Vector"
                                       src="https://c.animaapp.com/VPWnEuWR/img/vector-42.svg"
                                       width={98}
                                       height={38}
                                    />
                                    <div className="absolute w-[255px] h-14 top-[11px] left-9 [font-family:'Cabin_Condensed',Helvetica] font-bold text-white text-[40px] tracking-[0]">
                                       $ Repayment
                                    </div>
                                 </div>
                              </div>
                           </div>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
            <Image
               alt=""
               loading="lazy"
               src="/landing-image.png"
               className="absolute w-[34px] h-[34px] top-[413px] left-[275px]"
               width={34}
               height={34}
            />
            <Image
               className="absolute w-[221px] h-[65px] top-[325px] left-[555px]"
               alt="Vector"
               src="https://c.animaapp.com/VPWnEuWR/img/vector-44.svg"
               width={221}
               height={65}
            />
            <div className="top-[421px] left-[88px] absolute h-[22px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
               No Collateral
            </div>
            <div className="top-[488px] left-[88px] absolute h-[22px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
               No Deposit
            </div>
            <div className="top-[552px] left-[88px] absolute h-[22px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
               No Fees
            </div>
            <div className="absolute h-[22px] top-[348px] left-[585px] [font-family:'BB_Torsos_Pro-Medium',Helvetica] font-bold text-white text-lg tracking-[0] leading-[normal]">
               Super Competitive
            </div>
         </div>
      </div>
   );
}
