...
export default function Card({ type, loan }: { type: boolean; loan: Loan }) {
   ...
   resultDate.setDate(date.getDate() + loan.days);
   const differenceInDays = Math.round(loan.days - timeDifference / (1000 * 60 * 60 * 24));
   const dueDate = resultDate.toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
   });
   ...
   const getNetworkColor = (network: string) => {
      ...
      return (colors as Record<string, string>)[network] || 'bg-black';
   };

   const getNetworkSvg = (network: string) => {
      const colors = {
         solana: (
         ...
         optimism: (
            <svg width="20px" height="20px" viewBox="0 0 28 15" fill="none" xmlns="http://www.w3.org/2000/svg">
               <path
         ...
         arbitrum: (
            <svg width="20px" height="20px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2500 2500" fill="none">
      ...
      c0-68-36-130-95-164L1344,25c-29-17-62-25-95-25l0,0H1250z"
         ...
         polygon: (
            <svg width="20px" height="20px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 178 161" fill="none">
               <path
                  fill="#FFFFFF"
      ...
      return (colors as Record<string, ReactNode>)[network] || <i className="fas fa-gem text-white text-lg"></i>;
   };
   ...
   return type && loan.loanStatus === 'Lent' ? (
      <div className="bg-white rounded-xl max-w-[320px] w-full h-full flex flex-col shadow-md">
         <div className="p-5 pb-3 flex justify-between items-start">
            ...
            <button aria-label="Diamond icon" className={`${getNetworkColor(loan.block)} rounded-r-md rounded-l-md px-3 py-2`}>
               {getNetworkSvg(loan.block)}
            </button>
         </div>
      ...
      <div className="bg-white rounded-xl max-w-[320px] w-full h-full flex flex-col shadow-md">
         ...
         <div
            className={
               'text-center text-[14px] leading-[18px] font-normal py-2 border-t border-b ' +
               (loan.loanStatus === 'Lent' && loan.repaymentStatus !== 'Paid'
                  ? differenceInDays > 29
                     ? 'bg-[#D1FAE5] text-[#22C55E] border-[#A7F3D0]'
                     : differenceInDays > 6
                       ? 'bg-[#FEF3C7] text-[#D97706] border-[#FDE68A]'
                       : 'bg-[#FEE2E2] text-[#EF4444] border-[#FECACA]'
                  : 'border-[#EDE9FE] bg-[#F3E8FF] text-[#A78BFA]')
            }
         >
         ...
         {showPay && loan.repaymentStatus !== 'Paid' ? (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
               <button onClick={() => setShowPay(false)} className="text-gray-600 hover:text-gray-800 fixed top-4 right-4 z-50">
                  ✖
               </button>
               <UserPay loan={loan} />
            </div>
         ...
         {showDel ? (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
               <button onClick={() => setShowDel(false)} className="text-gray-600 hover:text-gray-800 fixed top-4 right-4 z-50">
                  ✖
               </button>
               <main className="flex flex-col py-7 w-full bg-white rounded-3xl border border-solid border-neutral-200 shadow-[0px_2px_8px_rgba(0,0,0,0.25)] flex overflow-hidden flex-col py-5 bg-white rounded-2xl max-w-[473px]">
                  <section className="flex flex-col px-5 w-full">
                     <h1 className="self-start text-2xl font-medium leading-none text-black">Delete Loan Request?</h1>
                     <p className="mt-5 text-sm leading-6 text-gray-500 text-opacity-60">
                     ...
                     <div className="self-end">
                        ...
                        <button
                           ...
                           className="overflow-hidden flex-auto gap-5 self-stretch px-5 py-3.5 rounded-lg min-h-[46px] bg-red-500 text-white border border-solid"
                           type="button"
                        >
                           Delete Request
                        </button>
