...
function Card({ children, className = '', radius = '16px' }: { children: React.ReactNode; className?: string; radius?: string }) {
   return (
      <div className={`bg-[var(--surface)] relative ${className}`} style={{ boxShadow: 'var(--card-shadow)', borderRadius: radius }}>
         <div
            aria-hidden
            className="absolute inset-0 border border-[var(--border-card-2)] pointer-events-none"
...
function ReceiveEstimate({ currency, usdcAmount }: { currency: string; usdcAmount: number }) {
   ...
   return (
      <>
         <p className="text-[19px] font-semibold text-[var(--ink)] tracking-[-0.4px] mt-[4px]">
            ≈ {symbol}
            {val.toLocaleString(undefined, { maximumFractionDigits: 0 })}
         </p>
         <p className="text-[11px] text-[var(--text-faint)] leading-[15px] mt-[2px]">
            1 USDC ≈ {symbol}
...
function MoneybeesMark({ className = '' }: { className?: string }) {
   return (
      <svg viewBox="0 0 48 48" className={className} xmlns="http://www.w3.org/2000/svg">
         ...
         <path
            d="M24 3 L42.18 13.5 L42.18 34.5 L24 45 L5.82 34.5 L5.82 13.5 Z"
            fill="url(#mbGold)"
            stroke="#0A0A0A"
            strokeWidth="3"
...
function GCashMark({ className = '' }: { className?: string }) {
   return (
      <svg viewBox="0 0 100 100" className={className} xmlns="http://www.w3.org/2000/svg">
         ...
         <path
            d="M56 35 A18 18 0 1 0 56 65 L56 50 L43 50"
            fill="none"
            stroke="#0A2FA8"
            strokeWidth="11"
            strokeLinecap="round"
...
function HowToModal({ guide, onClose }: { guide: Guide; onClose: () => void }) {
   return (
      <div className="absolute inset-0 bg-black/40 flex items-end justify-center z-[60]" onClick={onClose}>
         <div
            className="w-full bg-[var(--surface)] rounded-t-[24px] p-[20px] space-y-[16px] pb-[max(20px,env(safe-area-inset-bottom))]"
            ...
            {guide.video ? (
               <div className="rounded-[16px] overflow-hidden aspect-video bg-black">
                  <iframe
                     ...
                     src={`https://www.youtube-nocookie.com/embed/${guide.video}`}
                     title={guide.title}
                     loading="lazy"
               ...
               <div className="rounded-[16px] bg-[var(--surface-1)] border border-[var(--border-3)] aspect-video flex flex-col items-center justify-center gap-[8px]">
                  <div
                     className="w-[50px] h-[50px] rounded-full bg-white/90 flex items-center justify-center"
                     style={{ boxShadow: '0px 4px 12px rgba(96,16,210,0.15)' }}
                  >
            ...
            <div className="space-y-[12px]">
               {guide.steps.map((s, i) => (
                  <div key={i} className="flex gap-[10px]">
                     <div className="w-[22px] h-[22px] rounded-full bg-[var(--surface-1)] text-[var(--accent)] text-[12px] font-semibold flex items-center justify-center shrink-0 mt-[1px]">
                        {i + 1}
                     </div>
   ...
   const rows = steps.map((s, i) => (
      <div key={i} className="flex gap-[13px]">
         <div className="flex flex-col items-center shrink-0">
            <div className="w-[24px] h-[24px] rounded-full bg-[var(--surface-1)] border border-[var(--border-2)] text-[var(--primary)] text-[11px] font-bold flex items-center justify-center shrink-0">
               {i + 1}
            </div>
         ...
         <div className={`flex-1 min-w-0 ${i < steps.length - 1 ? 'pb-[16px]' : ''}`}>
            <div className="flex items-center gap-[7px] pt-[2px]">
               <p className="text-[15px] text-[var(--ink)] leading-[20px]" style={{ fontWeight: 590 }}>
                  {s.title}
               </p>
            </div>
...
function TimelineStep({
   ...
   return (
      <div className="flex gap-[14px]">
         <div className="flex flex-col items-center shrink-0">
            <div
               className={`w-[26px] h-[26px] rounded-full text-[12px] font-bold flex items-center justify-center z-10 transition-colors border ${
                  done
                     ? 'bg-[var(--green)] border-[var(--green)] text-white'
                     : 'bg-[var(--surface-1)] border-[var(--border-2)] text-[var(--primary)]'
               }`}
            >
...
function BaseOnlyNotice({ className = '' }: { className?: string }) {
   return (
      <div
         className={`rounded-[16px] bg-[var(--surface-2)] border border-[var(--border-1)] p-[16px] flex items-start gap-[12px] ${className}`}
      >
         <ShieldAlert className="w-[20px] h-[20px] text-[var(--accent)] shrink-0 mt-[1px]" />
         <p className="text-[13px] leading-[21px] text-[var(--text-2)]">
            Send only <UsdcMark className="inline-block w-[15px] h-[15px] align-[-3px] mx-[1px]" />{' '}
            <span className="font-semibold text-[var(--ink)]">USDC</span> on{' '}
            <BaseMark className="inline-block w-[14px] h-[14px] align-[-3px] mx-[1px]" />{' '}
            <span className="font-semibold text-[var(--ink)]">Base</span>. A different coin or network can't be recovered.
         </p>
      </div>
   );
}
...
function PickerRow({ id, selected, onSelect, icon, name, line1, line2, recommended, warn }: PickerRowProps) {
   ...
   return (
      <button onClick={() => onSelect(id)} className="relative w-full text-left outline-none">
         ...
         <div
            className={`rounded-[18px] p-[14px] border-2 flex items-center gap-[14px] transition-all ${active ? 'bg-[var(--surface-2)] border-[var(--primary)]' : 'bg-[var(--surface)] border-[var(--border-card)] hover:border-[var(--border-4)]'}`}
            style={{ boxShadow: active ? '0px 4px 14px rgba(96,16,210,0.10)' : '0px 1px 3px rgba(27,28,29,0.05)' }}
         >
            <div className="shrink-0">{icon}</div>
            <div className="flex-1 min-w-0">
               <p className="text-[16px] text-[var(--ink)] leading-[20px]" style={{ fontWeight: 590 }}>
                  {name}
               </p>
            ...
            <div
               className={`w-[22px] h-[22px] rounded-full border-2 flex items-center justify-center shrink-0 ${active ? 'bg-[var(--primary)] border-[var(--primary)]' : 'border-[var(--border-input)]'}`}
            >
      ...
      <div className="absolute inset-0 bg-[var(--surface)] flex flex-col">
         <div className="flex-1 overflow-y-auto px-[22px] pt-[max(44px,env(safe-area-inset-top))]">
            <div className="relative flex flex-col items-center text-center pt-[8px] pb-[6px]">
               ...
               <div
                  className="w-[64px] h-[64px] rounded-full bg-[var(--surface)] border-[5px] border-[var(--border-1)] flex items-center justify-center mb-[14px]"
                  style={{ boxShadow: '0px 6px 18px rgba(96,16,210,0.14)' }}
               >
               ...
               <h1 className="text-[27px] text-[var(--ink)]" style={{ fontWeight: 590, lineHeight: 1.1, letterSpacing: '-0.04em' }}>
                  Withdraw your USDC
               </h1>
            ...
            <p className="text-[18px] text-[var(--ink)] mt-[10px] mb-[6px]" style={{ fontWeight: 590, letterSpacing: '-0.02em' }}>
         ...
         <div className="px-[22px] pt-[12px] pb-[max(14px,env(safe-area-inset-bottom))] bg-[var(--surface)] border-t border-[var(--divider)] space-y-[6px]">
            ...
            <button
               onClick={onLater}
               className="w-full text-center text-[14px] font-semibold text-[var(--primary)] py-[6px] hover:underline"
            >
               I'll do this later
            </button>
         </div>
   ...
   return (
      <Card className="p-[18px]">
         <div className="flex items-center gap-[14px]">
            <div
               className={`w-[44px] h-[44px] rounded-full flex items-center justify-center shrink-0 ${failed ? 'bg-[var(--danger-bg)]' : 'bg-[var(--surface-1)]'}`}
            >
               {failed ? (
         ...
         {(status === 'delayed' || failed) && (
            <div className="mt-[12px] flex items-center justify-between gap-[10px] flex-wrap">
               <button
                  onClick={() => openSupportContacts('general')}
...
/* ─── Inline success banner shown after on-chain confirmation ───── */
...
function SuccessBanner({ exchange, amount }: { exchange: string; amount: number }) {
   return (
      <Card className="overflow-hidden">
         <div className="bg-[var(--green-bg)] px-[16px] py-[14px] flex items-center gap-[14px]">
            <div
            ...
            <div className="flex-1 min-w-0">
               <p className="text-[16px] font-semibold text-[var(--ink)] tracking-[-0.4px]">Sent to {exchange}!</p>
               <p className="text-[13px] text-[var(--text-muted)] leading-[18px] mt-[2px]">
                  {amount} USDC is on its way — arriving in a few minutes.
               </p>
            </div>
         </div>
      </Card>
...
function AppFlow({ cfg, onConfirmed, onDone }: { cfg: AppFlowConfig; onConfirmed: (amount: number) => void; onDone: () => void }) {
   ...
   useEffect(() => {
      if (sentStatus === 'arrived') {
         track('withdraw_confirmed', { exchange: cfg.name, amount: sentAmountRef.current });
         onConfirmed(sentAmountRef.current);
   ...
   return (
      <div className="space-y-[12px]">
         ...
         {arrived ? (
            <SuccessBanner exchange={cfg.name} amount={sentAmountRef.current} />
         ) : sentStatus === 'in-progress' || sentStatus === 'delayed' || sentStatus === 'failed' ? (
            <SentStatusCard
               exchange={cfg.name}
               amount={sentAmountRef.current || amtNum}
               status={sentStatus}
               txHash={txHash}
               isPreview={isPreview}
               onRetry={reset}
            ...
            <>
               <AmountCard
                  ...
                  payout={
                     <>
                        Payout to <span className="font-semibold text-[var(--ink)]">{cfg.payout}</span>
                     </>
                  }
               />
               ...
               <Card className="p-[16px] space-y-[12px]">
                  ...
                  <div className="space-y-[6px]">
                     <div
                        className="bg-[var(--surface)] rounded-[12px] relative overflow-hidden flex items-center"
                        style={{ boxShadow: '0px 2px 4px rgba(27,28,29,0.04)' }}
                     >
                        <div
                           aria-hidden
                           className={`absolute inset-0 rounded-[12px] border pointer-events-none ${address && !addrValid ? 'border-[var(--danger)]' : 'border-[var(--border-strong)]'}`}
                        />
                        <input
                           value={address}
                           onChange={(e) => setAddress(e.target.value)}
                           placeholder={`Paste your ${cfg.short} address`}
                     ...
                     {address && !addrValid && (
                        <p className="text-[12px] text-[var(--danger)] leading-[18px] flex items-center gap-1">
                           ...
                           Doesn't look like a valid wallet address.
                        </p>
                     )}
                  </div>
                  <div className="space-y-[6px]">
                     ...
                     >
                        <div
                           aria-hidden
                           className="absolute inset-0 rounded-[12px] border border-[var(--border-strong)] pointer-events-none"
                        />
                        <input
                           type="number"
                           value={amount}
                        ...
                        <button
                           onClick={() => {
                              if (spendable != null) setAmount(String(spendable));
                           }}
                           disabled={spendable == null}
                           className="text-[12px] text-[var(--accent)] font-semibold pr-[10px] shrink-0 hover:underline disabled:opacity-40 disabled:no-underline"
                        >
                           Max
                        </button>
         ...
         <Card className="overflow-hidden">
            <button onClick={() => setShowCashOut((v) => !v)} className="w-full flex items-center gap-[12px] px-[16px] py-[14px] text-left">
               ...
               <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[15px] text-[var(--ink)] tracking-[-0.3px]" style={{ fontWeight: 590 }}>
                     {cfg.cashOutTitle}
                  </p>
               </div>
               <ChevronRight
                  className={`w-[18px] h-[18px] text-[var(--border-strong)] shrink-0 transition-transform ${showCashOut ? 'rotate-90' : ''}`}
               />
            </button>
            {showCashOut && (
                  ...
                  {cfg.cashOutVideo && (
                     <div className="mb-[14px] rounded-[12px] overflow-hidden">
                        <iframe
                           className="w-full"
                           height="195"
                           src={`https://www.youtube.com/embed/${cfg.cashOutVideo}`}
   ...
   steps: [
      {
         ...
         title: 'Open GCrypto in GCash',
         desc: 'Open GCrypto, then tap Receive. Choose USDC and select Base as the network.',
         guide: {
            title: 'How to open GCrypto in GCash',
            video: 'm5-cD7v4SLg',
            link: { label: 'Official GCash guide', url: GCASH_HELP },
            steps: [
      ...
      },
      {
         ...
         title: 'Copy your GCash address',
         desc: 'Tap to copy the address shown on the screen.',
         guide: {
            title: 'How to copy your GCrypto address',
            video: 'm5-cD7v4SLg',
            link: { label: 'Official GCash guide', url: GCASH_HELP },
            steps: ['GCrypto shows your address.', 'Tap to copy the address.', 'Come back to Moodeng and paste it below.']
         }
      },
      {
   ...
   cashOutSteps: [
      {
         title: (
            <>
               In GCrypto, open <span className="font-bold">USDC</span> and tap <span className="font-bold">Sell</span>
            </>
         ),
      ...
      },
      {
         title: (
            <>
               Go to your <span className="font-bold">trading wallet</span> and tap <span className="font-bold">Withdraw</span>
            </>
         ),
...
const PDAX_DEPOSIT_HELP =
   'https://support.pdax.ph/support/solutions/articles/1060000097399-how-to-deposit-cryptocurrency-into-your-pdax-wallet-';
const PDAX_FLOW: AppFlowConfig = {
   ...
   steps: [
      {
         icon: <Download className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Open PDAX → Portfolio → USDC → Receive',
         desc: 'Select USDC, tap Receive, then choose Base as the network.',
         guide: {
            title: 'How to find your PDAX receiving address',
            video: 'q8F8H1UzBEU',
            link: { label: 'Official PDAX guide', url: PDAX_DEPOSIT_HELP },
            steps: [
      ...
      {
         icon: <Copy className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Copy your PDAX address',
         desc: 'Tap to copy the address from the screen.',
         guide: {
            title: 'How to copy your PDAX address',
            video: 'q8F8H1UzBEU',
            link: { label: 'Official PDAX guide', url: PDAX_DEPOSIT_HELP },
            steps: ['PDAX shows your receiving address.', 'Tap to copy the address.', 'Come back to Moodeng and paste it below.']
         }
      },
      {
         icon: <ClipboardCheck className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
   ...
   cashOutSteps: [
      {
         title: (
            <>
               In PDAX, go to <span className="font-bold">Trade → Sell</span>
            </>
         ),
      ...
      },
      {
         title: (
            <>
               Select <span className="font-bold">USDC → PHP</span> as the pair
            </>
         ),
      ...
      },
      {
         title: (
            <>
               Go to <span className="font-bold">Wallet → Withdraw PHP</span>
            </>
         ),
         helper:
...
const COINSPH_FLOW: AppFlowConfig = {
   ...
   steps: [
      {
         icon: <Download className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Open Coins.ph → Portfolio → USDC → Receive',
         desc: 'Go to your Portfolio, select USDC, then tap Receive. Choose Base as the network.',
         guide: {
            title: 'How to find your Coins.ph receiving address',
            video: '8jgQIuqp5E4?start=45',
            link: { label: 'Official Coins.ph guide', url: COINSPH_DEPOSIT_HELP },
            steps: [
      ...
      {
         icon: <Copy className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Copy your Coins.ph address',
         desc: 'Tap to copy the address shown on screen.'
      },
      {
         icon: <ClipboardCheck className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Paste it below',
   ...
   cashOutSteps: [
      {
         title: (
            <>
               Tap <span className="font-bold">Trade → Sell</span>, then select <span className="font-bold">USDC</span>
            </>
         ),
      ...
      },
      {
         title: (
            <>
               Go to <span className="font-bold">Portfolio → Withdraw / Cash Out</span>
            </>
         ),
      ...
      },
      {
         title: (
            <>
               Select transfer type: <span className="font-bold">InstaPay</span> or <span className="font-bold">PESONet</span>
            </>
         ),
...
function BinanceFlow({ onConfirmed, onDone }: { onConfirmed: (amount: number) => void; onDone: () => void }) {
   ...
   useEffect(() => {
      if (sentStatus === 'arrived') {
         track('withdraw_confirmed', { exchange: 'Binance', amount: sentAmountRef.current });
         onConfirmed(sentAmountRef.current);
            ...
            <>
                  ...
                  payout={
                     <>
                        ...
                        <span className="font-semibold text-[var(--ink)]">
                           {isPH ? 'GCash, Maya or Bank' : 'your bank or local wallet'}
                        </span>
                     </>
                  }
               />
               ...
               <div>
                  ...
                  <StepList
                     steps={[
                        {
                           title: 'Open Binance → Wallet → Receive',
                           desc: 'Tap Wallet, then Receive. Search for USDC and choose Base as the network.',
                           guide: {
                              title: 'How to find your Binance receiving address',
                              video: 'KAuoySzS0Mc',
                              steps: [
                        ...
                        {
                           title: 'Copy your Binance address',
                           desc: 'Tap Copy Address to copy it.',
                           guide: {
                              title: 'How to copy your Binance address',
                              video: 'I-IYLoBTdEQ',
                              steps: [
                  ...
                  <div className="space-y-[6px]">
                     ...
                     >
                        ...
                        <input
                           value={address}
                           onChange={(e) => setAddress(e.target.value)}
                           placeholder="Paste your Binance address"
                           className="flex-1 px-[16px] py-[12px] bg-transparent text-[16px] leading-[24px] tracking-[-0.32px] text-[var(--ink)] placeholder:text-[var(--text-muted)] focus:outline-none w-full"
                        />
                        {address && (
                           <button onClick={() => setAddress('')} className="pr-[12px] text-[var(--text-muted)] hover:text-[var(--ink)]">
         ...
         <Card className="overflow-hidden">
            <button onClick={() => setShowP2P((v) => !v)} className="w-full flex items-center gap-[12px] px-[16px] py-[14px] text-left">
               ...
               <div className="flex-1 min-w-0">
                  <p className="text-[11px] font-semibold text-[var(--accent)] uppercase tracking-[0.4px]">After it arrives</p>
                  <p className="text-[15px] text-[var(--ink)] tracking-[-0.3px]" style={{ fontWeight: 590 }}>
                     {isPH ? 'Cash out to pesos with Binance P2P' : 'Cash out to local currency with Binance P2P'}
                  </p>
               </div>
               <ChevronRight
                  className={`w-[18px] h-[18px] text-[var(--border-strong)] shrink-0 transition-transform ${showP2P ? 'rotate-90' : ''}`}
               />
            </button>
            {showP2P && (
               <div className="px-[16px] pb-[16px] pt-[2px] border-t border-[var(--surface-grey)]">
                  <div className="rounded-[14px] overflow-hidden aspect-video bg-black mt-[14px]">
                     <iframe
                        className="w-full h-full"
                        src="https://www.youtube-nocookie.com/embed/cld4bNfixTc"
                        title="Binance P2P walkthrough"
                        loading="lazy"
                        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                        allowFullScreen
                     />
                  ...
                  <p className="text-[13px] leading-[19px] tracking-[-0.26px] text-[var(--text-muted)] pt-[12px] pb-[14px]">
                     Once your {amount || LOAN_USDC} USDC shows in Binance, sell it for {isPH ? 'pesos' : 'local currency'} to a verified
                     buyer — Binance holds the crypto in escrow until you're paid.
                  </p>
                  <TimelineStep
                     n={1}
                     title={
                        <>
                           Open <span className="font-bold">P2P Trading → Sell → USDC</span>
                        </>
                     }
                  />
                  <TimelineStep
                     ...
                     title={
                        isPH ? (
                           <>
                              Choose <span className="font-bold">PHP</span> and your payout method
                           </>
                        ) : (
                           <>
                              Choose your <span className="font-bold">local currency</span> and payout method
                           </>
                        )
                     }
                  >
                     <p className="text-[13px] text-[var(--text-muted)] leading-[19px] tracking-[-0.26px]">
                        {isPH ? 'GCash, Maya, or bank transfer (BDO, BPI, etc.).' : 'Bank transfer or local payment method.'}
                     </p>
                  </TimelineStep>
                  <TimelineStep n={3} title="Pick a trustworthy buyer">
                     <p className="text-[13px] text-[var(--text-muted)] leading-[19px] tracking-[-0.26px]">
                        Prefer a <span className="font-semibold text-[var(--ink)]">95%+ completion rate</span> and many completed trades.
                     </p>
                  </TimelineStep>
                  ...
                  <TimelineStep
                     n={5}
                     last
                     title={isPH ? 'Release USDC only after pesos arrive' : 'Release USDC only after payment arrives'}
                  >
                     <div className="bg-[var(--danger-bg)] rounded-[8px] px-[10px] py-[6px] border border-[var(--danger-border)]">
                        <p className="text-[12px] leading-[18px] tracking-[-0.24px] text-[var(--danger-text)]">
                           <span className="font-bold">Never</span> tap "Release" until the{' '}
                           {isPH ? 'peso payment is actually in your GCash/bank' : 'payment is actually in your account'}. Check it yourself
                           first.
                        </p>
                     </div>
                  </TimelineStep>
...
function TelegramIcon({ className = '' }: { className?: string }) {
   return (
      <svg viewBox="0 0 24 24" className={className} xmlns="http://www.w3.org/2000/svg">
         ...
         <path
            fill="#fff"
            d="M5.5 11.8 17.2 7.3c.54-.2 1.01.13.84.95l-2 9.43c-.14.66-.53.82-1.08.51l-2.99-2.2-1.44 1.39c-.16.16-.3.3-.6.3l.21-3.06 5.56-5.02c.24-.21-.05-.33-.38-.12L8.1 13.2l-2.96-.92c-.64-.2-.66-.64.14-.95z"
         />
      </svg>
   );
}
function ViberIcon({ className = '' }: { className?: string }) {
   return (
      <svg viewBox="0 0 48 48" className={className} xmlns="http://www.w3.org/2000/svg">
         ...
         <path
            fill="#fff"
            d="M21 17.4c-.8.1-1.4.9-1.3 1.7.6 4.6 4.3 8.3 8.9 8.9.8.1 1.6-.5 1.7-1.3l.2-1.9c.1-.6-.3-1.2-.9-1.5l-2.1-.9c-.5-.2-1.1-.1-1.5.3l-.7.7c-1.5-.8-2.7-2-3.5-3.5l.7-.7c.4-.4.5-1 .3-1.5l-.9-2.1c-.3-.6-.9-1-1.5-.9z"
         />
         <path
            fill="none"
            stroke="#fff"
...
function MoneybeesExternalFlow() {
   ...
   const steps: FlowStep[] = [
      {
         icon: <Landmark className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Visit moneybees.ph',
         desc: 'Start from their official website and follow their process.'
      },
      {
         icon: <ClipboardCheck className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Arrange your cash-out with them',
         desc: 'Moneybees handles ID checks, the rate, and the payout directly with you.'
      },
      {
         icon: <Lock className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Send only after their instructions',
         desc: `Send your ${LOAN_USDC} USDC only once Moneybees confirms the details.`
      }
   ];
   ...
   return (
      <div className="space-y-[12px]">
         <HowThisWorks>
            Moneybees is an external option some users may use to buy or sell crypto through Moneybees&rsquo; own process. It isn&rsquo;t
            operated by Moodeng — you&rsquo;ll need to follow Moneybees&rsquo; instructions directly.
         </HowThisWorks>

         <div>
            <p className="text-[16px] text-[var(--ink)] mb-[8px]" style={{ fontWeight: 590, letterSpacing: '-0.02em' }}>
               How it works
            </p>
...
function MoneybeesIntegratedFlow() {
   ...
   const steps: FlowStep[] = [
      {
         icon: <CreditCard className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Complete Moneybees KYC',
         desc: 'A one-time ID check on their secure page.'
      },
      {
         icon: <MessageCircle className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Get assisted by chat',
         desc: 'Via Telegram, Viber, or WhatsApp.',
         extra: (
      ...
      },
      {
         ...
         desc: 'They lock in the rate and payout method.'
      },
      {
         icon: <Lock className="w-[19px] h-[19px] text-[var(--accent)]" strokeWidth={2.2} />,
         title: 'Send only after instructions',
         desc: 'Send USDC only once Moneybees confirms the details.'
      }
   ];
   ...
   return (
      <div className="space-y-[12px]">
         <AmountCard
            ...
            payout={
               <>
                  Payout to <span className="font-semibold text-[var(--ink)]">Bank</span>,{' '}
                  <span className="font-semibold text-[var(--ink)]">GCash</span>, or{' '}
                  <span className="font-semibold text-[var(--ink)]">Maya</span>
               </>
            }
         />

         <HowThisWorks>
            Moneybees is a BSP/AMLC-registered cash-out provider. They handle KYC, exchange rate, transaction details, and PHP payout
            directly with you.
         </HowThisWorks>
         ...
         {phase === 'done' ? (
            <div className="rounded-[14px] bg-[var(--green-bg)] border border-[var(--green-border)] p-[16px] flex gap-[12px]">
               ...
               <div>
                  ...
                  <p className="text-[13px] text-[var(--green-text)] leading-[19px] mt-[2px]">
                     Moneybees will message you on your chosen chat app to confirm the rate and complete your {LOAN_USDC} USDC cash-out.
                  </p>
               </div>
            </div>
         ) : phase === 'pending' ? (
            <div className="space-y-[10px]">
               <div className="bg-[var(--amber-bg-2)] rounded-[12px] px-[14px] py-[12px] flex gap-[8px]">
                  ...
                  <p className="text-[13px] leading-[19px] tracking-[-0.26px] text-[var(--amber-text)]">
                     Finish verifying on the Moneybees page that opened, then tap below. Once Moneybees confirms your identity, they'll
                     reach out by chat.
                  </p>
               </div>
               ...
               <button
                  ...
                  className="w-full text-[13px] text-[var(--accent)] font-semibold tracking-[-0.26px] py-[4px] text-center hover:underline"
               >
                  Reopen Moneybees verification
               </button>
            </div>
         ) : (
            <>
               <PrimaryBtn onClick={() => setPhase('pending')}>
                  Continue to Moneybees KYC <ArrowRight className="w-4 h-4" />
               </PrimaryBtn>
         ...
         <div className="flex-1 overflow-y-auto pb-[40px]">
            <div className="px-[16px] pt-[12px]">
               {provider === 'moneybees' ? (
                  ...
                  <BinanceFlow onConfirmed={onConfirmed} onDone={onDone} />
               ) : provider === 'gcash' ? (
...
export default function Withdraw() {
   ...
   const fundedLoans = useMemo(() => loans.filter((loan) => loan.borrowerUser === user.id && loan.loanStatus === 'Lent'), [loans, user.id]);
   ...
   return (
      <WithdrawDataContext.Provider value={data}>
         <div
            style={{ fontFamily: FONT, backgroundColor: 'var(--app-bg)' }}
            className="withdraw-flow fixed inset-0 z-50 overflow-hidden flex justify-center"
         >
