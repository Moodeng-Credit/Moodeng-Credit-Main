...
// Users almost never report a broken step; they just leave. On 2026-09-26 the wallet face check had
// been failing for days (Didit wanted a face on file before KYC) and ~20 borrowers dropped before
// anyone noticed. So any failure to start a check pings Discord #kyc straight away.
// deno-lint-ignore no-explicit-any
const alertStartFailure = async (supabase: any, userId: string | null, kind: string, detail: string) => {
   try {
      const { data } = userId ? await supabase.from('users').select('username').eq('id', userId).maybeSingle() : { data: null };
      await postDiscord(
         {
            embeds: [
               {
                  title: `⚠️ Couldn't start a ${kind} check`,
                  description: `**User:** ${data?.username ?? userId ?? 'unknown'}\n**Error:** ${detail.slice(0, 900)}\n\nThe user saw "Failed to create verification session". If this repeats, the step is broken for everyone.`,
                  color: 0xea580c,
      ...
      console.error('[create-didit-session] failure alert failed', err instanceof Error ? err.message : err);
   }
};
...
serve(async (req) => {
   ...
   try {
      ...
      if (!diditResponse.ok || !diditBody?.url) {
         ...
         await alertStartFailure(supabase, user.id, kind, `Didit ${diditResponse.status}: ${JSON.stringify(diditBody ?? {})}`);
