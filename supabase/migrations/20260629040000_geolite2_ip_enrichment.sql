-- GeoLite2 IP enrichment: store non-PII geo metadata alongside the hashed IP.
-- The raw IP is still never stored — MaxMind is queried at request time, metadata
-- is kept, then the IP is hashed and discarded.

-- ---------------------------------------------------------------------------
-- 1. New columns on auth_ip_log for geo metadata
-- ---------------------------------------------------------------------------
alter table public.auth_ip_log
...
--    the whitelist-aware version from the prior migration.
-- ---------------------------------------------------------------------------
...
begin
  ...
  for rec in
    select a.user_id, u.username, u.email,
           ...
           extract(epoch from abs(b.last_seen_at - a.last_seen_at)) / 3600.0 as hours_apart
