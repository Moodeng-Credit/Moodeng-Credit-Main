-- Fraud alerts to Telegram + cross-role IP detection.
--
-- 1. The daily fraud scan previously only emailed the founder. Alerts now also go
--    to a Telegram group: telegram_bot_settings key 'fraud_alert_chat_id', seeded
--    from 'kyc_alert_chat_id' (the KYC alerts group) so delivery starts without
--    extra setup. Point it at a different group by updating the setting.
--
-- 2. New signal H: cross_role_shared_ip. Existing IP checks only fire for the two
--    counterparties of an EXISTING loan (signal D) or for 3+ accounts in a subnet
--    (signal G). A borrower who opens a second account to lend — before any loan
--    links the two accounts — slipped through unless the wallets matched. Now a
--    borrower-role account and a lender-role account sharing a login IP within
--    the window raises a signal, deduped per account pair.

insert into public.telegram_bot_settings (key, value, description)
select 'fraud_alert_chat_id',
       coalesce((select value from public.telegram_bot_settings where key = 'kyc_alert_chat_id'), ''),
       'Telegram group chat id for fraud-scan security alerts. Seeded from kyc_alert_chat_id.'
on conflict (key) do nothing;

-- Function body below is identical to 20260710050000 plus new block H.
...
begin
  ...
  -- H. Cross-role IP overlap: a borrower-role account and a lender-role account
  --    (different users, NO loan between them required) sharing a login IP within
  --    the window. Catches the self-lending setup phase — a borrower opening a
  --    second account to lend — before signals B/C/D can see a loan linking them.
  --    Deduped per account pair; skip when both sides are whitelisted.
  for rec in
    select ub.id as borrower_user_id, ub.username as borrower_username, ub.email as borrower_email,
           ul.id as lender_user_id, ul.username as lender_username, ul.email as lender_email,
           count(distinct ia.ip_hash) as shared_ip_count
    from public.auth_ip_log ia
    join public.users ub on ub.id = ia.user_id and ub.user_role::text = 'borrower'
    join public.auth_ip_log ib on ib.ip_hash = ia.ip_hash and ib.user_id <> ia.user_id
    join public.users ul on ul.id = ib.user_id and ul.user_role::text = 'lender'
    left join public.fraud_detection_whitelist wb on wb.user_id = ub.id
    left join public.fraud_detection_whitelist wl on wl.user_id = ul.id
    where ia.last_seen_at >= now() - make_interval(days => ip_window_days)
      and ib.last_seen_at >= now() - make_interval(days => ip_window_days)
      and (wb.user_id is null or wl.user_id is null)
    group by ub.id, ub.username, ub.email, ul.id, ul.username, ul.email
  loop
    key := rec.borrower_user_id::text || ':' || rec.lender_user_id::text;
    if not exists (select 1 from public.fraud_signal_alerts a
                   where a.signal_type = 'cross_role_shared_ip' and a.subject_key = key) then
      insert into public.fraud_signal_alerts (signal_type, subject_key, details)
      values ('cross_role_shared_ip', key,
              jsonb_build_object('borrower_user_id', rec.borrower_user_id, 'borrower_username', rec.borrower_username,
                                 'borrower_email', rec.borrower_email,
                                 'lender_user_id', rec.lender_user_id, 'lender_username', rec.lender_username,
                                 'lender_email', rec.lender_email,
                                 'shared_ip_count', rec.shared_ip_count));
      new_signals := new_signals || jsonb_build_object(
        'type', 'cross_role_shared_ip', 'severity', 'warning',
        'borrower_user_id', rec.borrower_user_id, 'borrower_username', rec.borrower_username,
        'lender_user_id', rec.lender_user_id, 'lender_username', rec.lender_username,
        'shared_ip_count', rec.shared_ip_count);
    end if;
  end loop;
